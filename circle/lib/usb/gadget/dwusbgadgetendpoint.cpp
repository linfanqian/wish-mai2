...
:	m_pGadget (pGadget),
	...
	m_Type ((pDesc->bmAttributes & 0x03) == 3 ? TypeInterrupt : TypeBulk),
	m_nEP (pDesc->bEndpointAddress & 0xF),
	m_nMaxPacketSize (pDesc->wMaxPacketSize & 0x7FF)
{
	assert ((pDesc->bmAttributes & 0x03) == 2 || (pDesc->bmAttributes & 0x03) == 3);
...
void CDWUSBGadgetEndpoint::OnUSBReset (void)
{
	...
	else
	{
		...
		assert (m_Type == TypeBulk || m_Type == TypeInterrupt);
		EPCtrl.And (~DWHCI_DEV_EP_CTRL_EP_TYPE__MASK);
		u32 nEPType = (m_Type == TypeInterrupt) ? DWHCI_DEV_EP_CTRL_EP_TYPE_INTR
							: DWHCI_DEV_EP_CTRL_EP_TYPE_BULK;
		EPCtrl.Or (nEPType << DWHCI_DEV_EP_CTRL_EP_TYPE__SHIFT);
